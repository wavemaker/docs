...
function TabsComponent(props) {
  ...
  const handleTabChange = (event) => {
    ...
    if (event.currentTarget.id == "prev") {
      if (tabRefs.indexOf(currentElement) > 0) {
        newTab = tabRefs[tabRefs.indexOf(currentElement) - 1];
      }
      else {
        newTab = tabRefs[0];
      }
      ...
      if (tabRefs.indexOf(currentElement) >= 0 && tabRefs.indexOf(currentElement) < tabRefs.length - 1) {
        newTab = tabRefs[tabRefs.indexOf(currentElement) + 1];
      }
      else {
        newTab = tabRefs[tabRefs.length - 1];
      }
    }
    else {
    ...
    setCurrentElement(tabRefs[newTabValue]);
