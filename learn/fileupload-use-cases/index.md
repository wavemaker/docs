---
title: "FileUpload - Use Cases"
date: "2016-06-27"
---

widget can be used to let users upload files to your apps. WaveMaker  widget generates a **Service** to enable additional file processing functionality. It exposes few methods like , , and

- [Basic Usage](/learn/app-development/widgets/form-widgets/file-upload-basic-usage/)
- [to use various file service operations](/learn/how-tos/file-upload-widget-operations/)
- [to save uploaded file to a DB](/learn/how-tos/upload-file-save-database/)
- [to upload BLOB file to a DB](/learn/how-tos/file-upload-blob-data/)
- [to upload file to custom directory](/learn/how-tos/file-upload-custom-directory/)
- [to uploaded files from Java code](/learn/how-tos/accessing-file-upload-java-code/)
- [to use FileUpload in Form and Live Form](/learn/how-tos/upload-files-from-live-form-form/)

[![](../assets/fu_service.png)](../assets/fu_service.png)

[3\. Form Widgets](/learn/app-development/widgets/widget-library/#form)

- [3.1 Button](/learn/app-development/widgets/form/button/)
- [3.2 Button Group](/learn/app-development/widgets/form/button-group/)
- [3.3 Calendar](/learn/app-development/widgets/form/calendar/)
- [3.4 Checkbox](/learn/app-development/widgets/form/checkbox/)
- [3.5 CheckboxSet](/learn/app-development/widgets/form/checkboxset/)
- [3.6 Chips](/learn/app-development/widgets/form-widgets/chips/)
- [3.7 Color Picker](/learn/app-development/widgets/form/color-picker/)
- [3.8 Currency](/learn/app-development/widgets/form/currency/)
- [3.9 Date](/learn/app-development/widgets/form-widgets/date-time-datetime/)
- [3.10 Datetime](/learn/app-development/widgets/form-widgets/date-time-datetime/)
- [3.11 FileUpload](/learn/app-development/widgets/form/file-upload/)
    - [Features](/learn/app-development/widgets/form/file-upload/#features)
    - [Properties](/learn/app-development/widgets/form/file-upload/#properties)
    - [Events](/learn/app-development/widgets/form/file-upload/#events)
    - [Use Cases](/learn/app-development/widgets/form-widgets/fileupload-use-cases/)
- [3.12 Radioset](/learn/app-development/widgets/form/radioset/)
- [3.13 Rating](/learn/app-development/widgets/form/rating/)
- [3.14 Select](/learn/app-development/widgets/form/select/)
- [3.15 Select Locale](/learn/app-development/widgets/form/select-locale/)
- [3.16 Slider](/learn/app-development/widgets/form/slider/)
- [3.17 Switch](/learn/app-development/widgets/form/switch/)
- [3.18 Text](/learn/app-development/widgets/form/text/)
- [3.19 Textarea](/learn/app-development/widgets/form/textarea/)
- [3.20 Time](/learn/app-development/widgets/form-widgets/date-time-datetime/)
- [3.21 Toggle](/learn/app-development/widgets/form/toggle/)
