...
(not available for color, date, time, month, week types) | Defines the maximum number of characters that can be entered in the editor. |
| Minimum value

(only for numeric, date, time, week, month types) | Enter minimum number. This property applies when the type is a number. |
| Maximum value

(only for numeric, date, time, week, month types) | Enter maximum number. This property applies when the type is a number. |
| Step

(only for numeric, date, time, week, month types) | Use the stepper to increment/decrement the input value by the specified step interval. Example: if step='3', legal numbers could be -3, 0, 3, 6, etc. |
| **Behavior** |
