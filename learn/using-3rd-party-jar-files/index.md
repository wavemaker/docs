...
1. _Utilities_, access **Explorer** [![](../assets/ext_import.png)](../assets/ext_import.png)
2. **Resource**, and select the folder to place the jar file to be imported. As an example, we are using a [](../assets/mail.zip)file. [![](../assets/jar_folder.png)](../assets/jar_folder.png)
3. the statements for the required classes in your Java Service file. In this example, we have used the , the package details of which can be found at [://javamail.java.net/nonav/docs/api/](https://javamail.java.net/nonav/docs/api/) To use this jar file to send emails we will be using the following code in the Java Service file after adding the import statements for the required class files:
...
4. invoke the Java Service create a new Variable for Java Service: [![](../assets/jar_sv.png)](../assets/jar_sv.png)
5. _parameters_ can be accessed from the tab and can be bound or set to static values
6. Java Service can be executed either
    - setting the **data on input change** to true thus triggering whenever the input values change or
    - triggering the Variable created for any event on a widget for example on Mouse Click event of a Button.
7. result from the execution of this service can be bound to any widget.
