...
# \-Services enabled Architecture

support micro-services architecture, WaveMaker uses an API-driven app development approach wherein REST APIs are generated automatically for the application. Every backend service such as Database Entity, Queries or Custom Java code, that is integrated into an application generates REST APIs. Some of these APIs are available for further customization based on the app integration needs.
...
is a non-visual component that is used to call web services, custom database queries and Java methods. Variables can be invoked by Service Calls and by handling the call events. Using variable events ensures:

- service call has returned.
- returned from the service calls are handled.
- result set is available.

### \-cycle of Variable

graphic below represents the life-cycle of the variable.

[![](../assets/sevice_var_lifecycle.png)](../assets/sevice_var_lifecycle.png)

1. user invokes the variable call through an action like click on a button.
2. event is emitted and event controller watches over the event.
3. controller requests the variable information from the base service.
4. controller calls the update method on the “Variable” service.
5. Service makes a call to the rest service along with the parameters passed.
6. response is returned which is handled by the Service. It also invokes the events after getting the data.
7. response data is updated in the variable
8. widgets listening to the data change on the variable gets updated.

# Data - Variable Events

### \-cycle Events of Variable

the life cycle of a variable, next step would be to capture and control the flow to include validation, pre and post processing of data. A typical event flow when a variable is used to update data would be as follows: [![](../assets/LSV_eventcycle.png)](../assets/LSV_eventcycle.png)

# APIs

mentioned earlier, each of the services integrated into your app is converted to a RESTful service and are consumed through their respective REST APIs.These REST APIs are exposed via the [Designer](http://[supsystic-show-popup id=110]) and can be re-configured as per the application needs.
...
- [5.1 Overview](#)
    - [Accessing Data](#accessing-data)
        - [Life-cycle of data](#life-cycle)
    - [Manipulating Data](#manipulating-data)
        - [Life-cycle of Events](#life-cycle-events)
    - [REST APIs](#rest-apis)
- 5.2 Web Services
