...
1. the **Utilities** option select **Explorer** Here you will find the option to import resources to your app.
2. **+** to open the **Resource** dialog box
3. the appropriate folder, and **Files** that need to be imported. You can select multiple files to import. : Be sure you select the proper folder, this will be used by the binding dialog to filter the files. For example, if you are using an Image widget, you will be selecting the Image type resource and WaveMaker will display only the files from the Images folder. [![](../assets/ext_resources_binding.png)](../assets/ext_resources_binding.png)
4. can create your own sub-folders. You can also create a nested folder structure by using / as the file separator. For example, to create images sub-folder under style folder in resources, select resources enter _/style_ under folder name field.
5. upon the resource imported its usage would differ.
    - the case of media resources like images, audio files or video files, you can bind them to the appropriate widgets.
    - the case of JavaScript resource, entry into the file and invoking the script function would require some JavaScript knowledge.
...
- 5.2 Web Services
    ...
    - [Service Variables](/learn/services/web-services/web-services/#service-variable)
...
- [5.6 3rd Party Libraries](#)
    - [Overview](#)
    - [Including resource files](#resource-files)
