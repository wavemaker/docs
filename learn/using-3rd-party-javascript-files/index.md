...
We will explain the process with the help of an example. We want to add a multiple date selection facility to our application. The current calendar widget allows selection of a single date. We will use [v1.6.3](../assets/MultiDatesPicker-v1.6.3.zip) jQuery UI downloaded from [://multidatespickr.sourceforge.net/](http://multidatespickr.sourceforge.net/) We will also be needing [\-ui-1.12.0.custom](../assets/jquery-ui-1.12.0.custom.zip) file. Download and unzip both the files

1. **Utilities**, select File Explorer and **Resource** We need to import two files:
    1. _file_ - jquery-ui.multidatespicker.js with the code and
    2. _file_ - mdp.css for the UI. The js file will be placed in the _/javascript_ folder and the CSS file in the _/css_ folder. Create the appropriate folders [![](../assets/js_import.png)](../assets/js_import.png)
...
3. the main page mark the position for the display of the date-picker using a div tag
    ...
    [![](../assets/js_markup.png)](../assets/js_markup.png) In the script tab attach the  function from the js file imported to the div created
...
- [5.6 3rd Party Libraries](/learn/app-development/services/3rd-party-libraries/)
    ...
    - [Using third-party JavaScript file](#)
