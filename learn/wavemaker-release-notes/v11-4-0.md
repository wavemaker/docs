...
Re-importing an API in an application can affect the application's behavior due to the API update. The API ReImport Impact Analysis feature provides a detailed report on how updating a target API can affect the application's program elements, such as widgets, variables, and more. Impact analysis report can greatly improve the developer experience, especially during iterative development.
