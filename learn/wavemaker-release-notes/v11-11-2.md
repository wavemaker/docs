...
Previously, the **Date** widget did not support dynamic binding for the **Date Pattern** property — it could only be set using a static value. This limitation restricted users from implementing more flexible scenarios. We haveve now added support for dynamic binding of the Date Pattern property. The same enhancement applies to the **Time** and **Datetime** widgets also.
