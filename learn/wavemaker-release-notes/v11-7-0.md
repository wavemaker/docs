...
WaveMaker is now honoring Corporate Proxy server configuration for enhanced connectivity. For example, the user can configure proxy servers to access the external APIs without having to expcilty expose to server directly.
...
- Fixed an issue where the users do not need to provide case-sensitive details in the project.
