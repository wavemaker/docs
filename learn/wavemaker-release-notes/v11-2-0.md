...
### True Single Page App Behavior

WaveMaker app's UI now behaves closer to a Single Page App functionality. When you navigate between pages, affecting only the content area to render again. The static area, such as header, leftnav, footer, etc., remains unaffected, providing smoother page transitions and a better user experience.

This feature is still in beta, so it is not enabled by default. 
