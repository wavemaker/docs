...
WaveMaker 11.7.1 is here with a new feature, Pull Request (PR) Flow. This also includes some exciting enhancements like Internationalization (i18n) message customization, extended Semantic Versioning, disabling Prefab editing in a project, and other bug fixes.  
...
To support development collaboration, WaveMaker introduced developer branching. Now, when a developer pushes code, a dedicated development branch is created in the code repository. With this, the changes pushed by the developer can be reviewed instead of automatically merging the changes to the Remote Tracking Branch (RTB).

:::note
PR Flow is applicable for WaveMaker Enterprise (WME) users only.
:::
...
We have extended our support to to allow editing commit messages during the Core Project Upgrade and Branch merge. This means users can now add or modify the commit message in the same dialog used to finalize the merge process.
...
To streamline the development process, we're disabling in-app Prefab editing.  This means users will make code changes directly within the Prefab project and publish the updates for use in any application.

#### Internationalization Message Customization for Prefabs used in a Project

We've introduced a new dialog for editing internationalization (i18n) messages for Prefab system messages directly within your project, eliminating the need to manually edit the messages in the en.json or af.json files from File Explorer.

### Label Widget Enhancement

To improve accessibility for labels used within any widget, WaveMaker is replacing the `<label>` tag with more specific tags like `<h1> `to `<h6>` or `<p>` depending on the selected layout. By default, the `<p>` tag will be used if no layout is selected. 

:::note
Labels within forms will continue to function as before and are not affected by this change. For more details, you can refer to our comprehensive blog post [here](link).
:::
