...
:::note
PR Flow is applicable for WaveMaker Teams only.
:::
...
### Filter Data on Key Press

Added enhancement in Datatable widget, under Advanced Settings when Filter Mode is set to Search, **Filter on key press** is enabled to start data filter on key press instead of clicking on Search.
