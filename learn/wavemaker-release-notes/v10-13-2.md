...
- **Infinite scroll pagination**: You can retain the selected row details, including edited values, when scrolling to the extreme bottom for viewing more records.
- **On-demand scroll**: Have control of the expended rows inside Datatable that do not collapse automatically.
- Model variable data syncs with table data when changes are detected.
