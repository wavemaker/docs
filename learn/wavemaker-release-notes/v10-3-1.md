...
- When using Centrail Authentication Server (CAS) as Authentication provider for an app, you can now configure additional information to be pulled out of CAS for the logged in user. You can do this by creating a service variable using the `securityService/getSecurityInfo` API.
...
- When adding app version, it was allowed to add only minor version upto 0-9. Now, the range is extended to 1-99. For example, 10.19.
