...
```
The uploaded `eslintrc-override.js` file will be merged with the framework’s default ESLint configuration.The Issues tab will reflect results based on this combined configuration.
...
A new **Short By** field has been added to the widget’s advanced settings. When configured, clicking on a custom column will sort the data based on the specified value.
</details>
