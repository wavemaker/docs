...
and processors are available for all REST Services with proxy enabled. They are used to process the request and response of the REST Service at runtime.

**: ** is used to process the request before sending it to the server. Processing of request can be used to add/delete/modify request headers or update request body before hitting the Service End Point.

**:** This is used to process the response sent by the server before sending it to the client. Processing of response includes adding/removing/modifying response headers or modifying the response body.

Following is the Request life cycle for a REST Service in a WaveMaker app: [![](../assets/REST-request-response.png)](../assets/REST-request-response.png) following request and response processors are provided, by default, for every REST service integrated into WaveMaker Apps.

This process parses the defaultHeaders from the incoming request if present and passes it onto the target endpoint. This processor takes a set of as the input parameters. Eg: User-Agent, Content-Type, Accept etc.

This processor is used to add a headerPrefix to all response headers returned by the server. A set of response headers can be excluded from getting prefixed by giving as input to the processor.

This processor can be used to configure whether to include or exclude the cookies sent by the server to the client. The property can be set to:

- \- removes all cookies.
- \_SESSION - persists cookies in the user session with attribute “wm.cookieStore“
- \- sends all response cookies tothe client.

A is generated by WaveMaker wherein a list of Request-Response processors, as mentioned above, are injected. Sample restRuntimeConfigBean looks as shown below.
...
# Request and Response Processor properties in Wavemaker

In order to configure the request and response processors:

1. File Explorer, open _<serviceId>.spring.xml_ file (services/serviceName/src/serviceId.spring.xml) of the REST Service you wish to configure.
2. can configure properties of the request and response processors by editing property values of bean with id: “serviceId+RestRuntimeConfig”.
3. the Request-Response processors are enabled by default. In order to disable a processor set enabled flag to false.
...
1. a Java Service that implements the HttpRequestProcessor/HttpResponseProcessor. You can also import the Java Class directly into src/main/Java/<package\_path>. (Note this <package\_path> needs to be created and should be given when adding the Java Class Bean in the next step.)
2. the Java Class Bean to httpRequestProcessorList/httpResponseProcessorList of restRuntimeConfigBean, in the above mentioned _<serviceId>.spring.xml_ file (services/serviceName/src/serviceName.spring.xml) of the REST Service you wish to customize.

## to implement for creating custom request processor
...
**Case**: Consider the case where the logged in user tenant id values need to be added to the request header, the following is the Java implementation:

 class DefaultHeaderRequestProcessor implements HttpRequestProcessor {
   @Override
...
## to implement for creating custom response processor
...
HttpRequestProcessorContext has the following structure:
 ...
 class HttpRequestProcessorContext {
...
HttpResponseProcessorContext has the following structure:
 ...
 class HttpResponseProcessorContext {
