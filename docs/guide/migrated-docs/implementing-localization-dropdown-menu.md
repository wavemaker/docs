...
**Problem Statement**: How to implement Localization for the menu items. [Localization](/docs/guide/migrated-docs/localization-wavemaker-apps) enables you to render your app in multiple languages. WaveMaker automatically detects the user’s preferred locales (from the browser) and renders the application with the locale of the user’s preference if the application supports. The user can be given a choice to select the language using the Select Locale widget.
...
4. Drag and drop a Dropdown Menu widget on the canvas ([more on Dropdown Menu](/docs/user-interfaces/web/components/angular-components/navigation/dropdown-menu-use-cases))
...
6. Drag and drop a Select Locale widget onto the canvas. The Select Locale widget auto-populates the languages added from the I18N dialog as a drop-down. ([more on Select Locale widget](/docs/user-interfaces/web/components/angular-components/form-widgets/select-locale))
...
[How to enable user role-based access to menu items](/docs/guide/migrated-docs/restricting-menu-item-display-based-user-role)  
