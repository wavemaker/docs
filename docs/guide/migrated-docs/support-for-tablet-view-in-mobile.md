...
5. Design a [partial page](/docs/user-interfaces/web/concepts/overview) that contains the employee profile in detail with Employee ID Param. Bind this partial to the selected-employee based on the Employee ID Partial Param to display the Employee Profile. For more information, see [How to create a Partial](/docs/guide/components/how-to-configure-row-expansion-in-a-data-table#creating-a-partial).
