...
The above provided steps is for development level. You can apply the same for deployment/live phases. The only thing you should concern about the SSO, Audience URL's. You need to change the urls according to the domain wherever you are deploying. For more clarity please refer to [SAML Integration](/docs/apis-and-services/security/providers/saml-integration) and for more detailed guidance on SAML integration in Okta, refer to the official Okta documentation:
