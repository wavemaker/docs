...
1. Click on **Create** from the _Prefab_ tab of the Project Dashboard
...
7. From Project Configurations, choose Config Prefab under Settings:
    - From Resources, add the js files 
    
    Image: d3_resource.png
...
10. Save and Publish the Prefab. You can set the version for the Prefab and Publish it. Know more about publishing Prefabs from here.
11. Once approved by the EDN Admin, the Prefab will be available for use across the Projects. You can see the entry in the Artifacts list from the Developer Utilities on the Project Workspace. Import it to see the same in the Widget Toolbox of any Project within your workspace.
...
2. Enter a name and description for the Prefab
3. **SAVE** the project setting without making any changes
4. Import Resource to import the following resource files: nv-d3-min & nv-d3 file into the Resource folder of your Prefab (download and extract the files). Image: nvd3_import.png
...
3. **Drag and drop the Prefab** onto the canvas and bind its **dataset** property. For this example, we have bound it to the _HrdbDepartmentData dataset_, created from the Database CRUD APIs available on the import of sample database 

Image: nvd3_bind.png

4. **Preview** the app.
 
 Image: nvd3_app_run.png
