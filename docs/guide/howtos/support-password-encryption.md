...
- Create a Java service and mention the following method. This would take the plain text as input and generates “Salt” for every plain text that is being encrypted. Dont forget to include the import statement
```    
...
```    
- Create a variable for the above Java service and bind the input parameter of the Java service variable to the _`password text widget >> datavalue`_ of the registration form.
...
Image: encrypt_pwd_xml.png 

### STEP-3: Importing the Java File for Validating Password

Download the MyPasswordEncoder file, extract the file and store it on your device.

- Import the `myPasswordEncoder` Java file in the following path: `project/src/main/java/com/wavemaker/runtime/security/provider/database/` 
