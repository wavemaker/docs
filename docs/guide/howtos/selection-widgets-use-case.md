...
3. Write and save a query to retrieve the total number of employees from a given city, state, zip. Save it as EmpTotal.
    ...
    Image: selection_query.png
4. Drag and drop a Select, Radioset, Checkboxset and a Label widget onto the canvas. Image: selection_design.png
5. Create a Database Crud Variable for the Employee entity Image: selection_lv.png
6. Bind the widgets thus:
    - **Select**: bind it to the Employee - Set the datafield property to city and display field also to city. The display field is the value that the user sees while the datafield is the value that is internally represents the display value. In this case both are the same. Image: selection_sel_prop.png
    - Repeat the above two steps for **Radioset** and **Checkboxset**. Choose the appropriate properties. Image: selection_check_prop.png       Image: selection_radio_prop.png
7. Create a Database API Variable for the Query service Image: selection_sv.png
8. Bind variable data city, state and zip to the selected values of Select, Checkboxset and Radioset widgets. Image: selection_SV_data.png Image: selection_SV_bind.png
9. Bind the caption property of the Label widget to the output from the query Image: selection_text.png
