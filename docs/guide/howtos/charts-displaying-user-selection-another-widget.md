...
1. We are using the _Department_ DB available after importing the sample DB which ships with the product.

Image: db_apis_db.png
...
2. We have designed the canvas with a _Grid Layout_ with three rows. We will have
    ...
    Image: chart_sel_design.png
    
3. Create a Database CRUD Variable (say, _deptVar_) using the Department table
