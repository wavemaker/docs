...
2. You have the Metadata URL for the WaveMaker App: Steps to obtain the Metadata URL:
    1. You should have already created an app using WaveMaker integrated with SAML as security provider (see here for more).
    2. Configure ADFS Federation Metadata details in WaveMaker App. You can either use the URL or upload file. Image: adfs_wm_meta1.png?v=20
    3. From Service Provider Configuration, generate Keystore. Image: adfs_wm_meta2.png
    4. Preview the app and open the Security dialog again. Note the Metadata URL from the WaveMaker App: Image: adfs_url.png
...
3. From Tools select AD FS Management Image: adfs_1.png
4. In AD FS 2.0 Management Console select "Add Relying Party Trust" Image: adfs_2.png Image: adfs_3.png
5. Select "Import data about the relying party from a file" Image: adfs_4.png
6. Select the metadata.xml file from the WaveMaker app as mentioned earlier in the prerequisites section (ignore any warning messages). Image: adfs_5.png Image: adfs_6.png
7. On the "Ready to Add Trust" make sure that tab endpoints contain multiple endpoint values. If not, verify that your metadata was generated with HTTPS protocol URLs. Image: adfs_7.png Image: adfs_8.png
8. Leave "Open the Edit Claim Rules dialog" checkbox checked and finish the wizard. Image: adfs_9.png
9. Select "Add Rule", choose "Send LDAP Attributes as Claims" and press Next Image: adfs_10.png
10. Add NameID as "Claim rule name", choose "Active Directory" as Attribute store, choose "SAM-Account-Name" as LDAP Attribute and "Name ID" as "Outgoing claim type", finish the wizard and confirm the claim rules window, in ADFS 3.0 you might need to configure the Name ID as a Pass Through claim - As we are using Spring SAML Security it expects the Name ID Parameter in the Spring SAML Response. Image: adfs_11.png
11. Open the provider by double-clicking it, select tab Advanced and change "Secure hash algorithm" to SHA-1 Image: adfs_12.png
...
1. Get the deployment URL from Profile Configuration dialog Image: adfs_wm_deploy.png
