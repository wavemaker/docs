...
The temporal feature in the DB2 product enables you to accurately track information and data changes over time and provides an efficient and cost-effective way to address auditing and compliance requirements, without having to specifically code for the same (see here for Temporal Support). You can use the Data and Live Widgets to manipulate values from the temporal tables.
...
We are using the following DB2 Employees schema with System Time defined: Image: dbtemp_ex1.png

## Use Case

We will see the implementation of the following scenarios:

- The manager enters the employee details assigning them to departments ie insert/update data Image: dbtemp_ex2.png
- The manager wants to track the progression of a particular employee ie view historical data Image: dbtemp_ex3.png
- The manager wants to see the status of all Employees on a given date ie view period data. Image: dbtemp_ex4.png
...
2. Set the data source to the Employees table Image: dbtemp_ex2a_dt.png
3. Choose Inline Editable format with Basic pagination
4. We have selected Employee Id, Name and Department to be displayed
5. Preview the page, you will see the Employees table displayed wherein you can add NEW rows or edit existing rows. Image: dbtemp_ex2a.png
...
- From the Data tab, using the bind icon set the employeeId to EmployeesTable1 (Data Table name, might differ for your example) -> selecteditem -> employeeid. This will ensure that the History data for the selected employee is fetched. Image: dbtemp_ex3_var.png
...
- From the canvas
    ...
    Image: dbtemp_ex3_dialog.png
- Go to the parent Page (you can choose it from the Page Structure or from the widget breadcrumb at the bottom of the canvas) and using the Advanced Settings for the Data Table, add a Custom Column to open the Dialog created in the previous step Image: dbtemp_ex3_dt.png
- Preview the page, click the View Details for any Employee and the dialog with Historical data will be displayed Image: dbtemp_ex2.pngImage: dbtemp_ex3.png
...
4. Preview the page, when you select a date from the Date widget, you will see the corresponding rows displayed in the Data Table. Image: dbtemp_ex4.png
