...
---
In this document, we will describe how to use **OpenID authentication** in a WaveMaker application. 
...
sequenceDiagram
    ...
    participant IDP as Identity Provider<br/>(Google, Okta, OAuth)

    UA ->> APP: Access Application
    APP -->> UA: Authentication Request<br/>(OpenID provider details)

    UA ->> IDP: OpenID Authentication Request
    Note right of IDP: Validation of credentials

    IDP -->> UA: Successful Response<br/>(contains user details)

    UA ->> APP: Redirection to Application
    Note right of APP: Creation of session

    APP -->> UA: Session established<br/>(Role assigned)
    APP -->> UA: Application renders<br/>Authenticated State

```
## Configuring Open ID

1. Go to **Project** > **Security**.
![alt text](../assets/security-workspace-ss.png)
...
![alt text](../assets/security-provider-ss.png)

Once the Open ID provider is selected, you can configure the security specifications based on the provider.

:::note 
- For more information, see [Google as provider](https://cloud.google.com/solutions/security)
- For more information, see [Okta as provider](https://www.okta.com/)
...
{/*![alt text](../assets/identity-provider-info.png)*/}
...
**Database:** When you select **Database**, authentication is performed using Open ID and authorization content is retrieved from the database. This allows you to use username and password credentials from Open ID while retrieving role content from the database. For role configuration using database, [see here](#).
