...
### Step 2: Select SAML as the Authentication Provider

1. Choose **SAML** as the authentication provider.
2. Provide a unique **Provider Name**.
3. Specify whether SAML is the primary authentication mechanism.


### Step 3: Configure Identity Provider (IdP) Details

You can configure the IdP using **Metadata URL** or **Metadata XML**.

#### Metadata URL
Provide the URL exposed by the Identity Provider that returns SAML metadata.
...
> Note:  
> - Metadata URLs do **not** require alias or password  
> - The URL must be publicly accessible from the WaveMaker runtime  
> - If validation fails, ensure HTTPS access, valid certificates, and correct tenant configuration

#### Metadata XML
Alternatively, paste the full metadata XML content provided by the IdP.


### Step 4: Configure Service Provider (SP) Settings

WaveMaker automatically generates SP metadata, including:

- Entity ID  
- Assertion Consumer Service (ACS) URL  
- Single Logout (SLO) URL  

This metadata must be uploaded or registered with the Identity Provider to establish trust.


### Step 5: Attribute Mapping

Map IdP attributes to WaveMaker user fields:
...
Role mapping can be:
- Derived directly from SAML assertions, or
...
- Access levels and permissions are applied
...
security.providers.saml.keyStorePassword=azure-ad-saml
