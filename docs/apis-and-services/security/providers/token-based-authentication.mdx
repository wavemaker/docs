...
### Step 2: Select Token-Based Authentication

- Choose **JWS (JWT)** or **OPAQUE** as the authentication provider.  
- Provide a unique **Provider Name**.  
- Specify if it is the primary authentication mechanism.
...
Map token claims to WaveMaker user fields:
...
> WaveMaker can extract roles from the token or resolve them from a database using the principal identifier.

### Step 5: Configure Authorization

- Apply access levels and permissions based on roles.  
- Protect pages, services, and APIs.  
