...
The Grid Layout is the most basic and commonly used content layout in WaveMaker. It lets you divide the page content into rows and columns and place widgets inside these grid cells with ease.

A key advantage of the Grid Layout is its responsive behavior. WaveMaker automatically adjusts the layout based on the device being used, such as desktop, tablet, or mobile, ensuring a consistent user experience across screen sizes.

WaveMaker implements the [Bootstrap Grid system](https://www.w3schools.com/bootstrap/bootstrap_grid_system.asp) which is the 12-column system:

<img
  src="https://docs.wavemaker.com/learn/assets/files/gridlayout_bootstrap-10bad713245a05838d4e263722ee360a.png"
  alt="Grid Layout Bootstrap"
...
- ```<wm-layoutgrid>``` is the main container that defines a grid-based layout for the page content.
- ```<wm-gridrow>``` represents a horizontal row in the grid layout.
- ```<wm-gridcolumn>``` defines a vertical column within a grid row.
- A columnwidth value 6 means each column occupies half the available width.
- Since each row contains two columns with equal width, the layout is evenly split into two sections.
