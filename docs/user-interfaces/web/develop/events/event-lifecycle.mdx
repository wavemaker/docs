...
Event lifecycle can flow either top-down or bottom-up, depending on what triggered the eventwhether it was the system, an API, or a user action.
...
Page.mainpageDestroy = function ($event, widget) {
    ...
    Page.Variables.selectedItem.setData({});
