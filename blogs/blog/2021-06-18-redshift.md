...
![Redshift Architecture](/img/blogs/redshift-architecture.png)
...
From [version 10.7.0](#), WaveMaker has extended its support to allow a dedicated section for the Redshift database. 
...
![Redshift Table Options](/img/blogs/redshift-table-options.png)
...
The queries can also have parameters that you can dynamically pass during runtime. Below is an example query of the Employee table that has `employeeId` as a parameter. For more information, see [Working with Queries in WaveMaker](#).

![Redshift Query](/img/blogs/redshift-queries.png)
...
![Redshift Configure List](/img/blogs/redshift-configure-list.png)
...
Import the Redshift database by following the how-to’s doc [Connect To AWS Redshift Database](#).
